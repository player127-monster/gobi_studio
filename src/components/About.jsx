...
export default function About(){
    ...
    return <section className="flex bg-primary flex-row justify-center pb-10 pt-0">
        ...
        <div id="about" className="w-1/2 flex flex-col justify-center gap-2.5 text-textColor text-justify  font-sriracha italic font-normal leading-relaxed pb-8">
            <p className='text-3xl pl-20 text-white font-bold font-sans not-italic pb-5'>Legacy Through the Lens</p>
            <div className="flex flex-col pl-24 ">
                <p className='text-xl pb-4'>Gobi Studio is a premier photography studio located in the heart of Watavala, offering easy access and exceptional service to clients across the region.</p>
                <p className='text-xl  pb-4'>Founded in 2004, Gobi Studio brings over two decades of experience in delivering high-quality photography and cinematography services. Established by the visionary Dr. Timothy Weeraratne—one of Sri Lanka’s pioneers in the field—we proudly carry forward his legacy by combining artistic excellence with the latest technology.</p>
                <p className='text-xl pb-4'>We are a government-registered studio, authorized to provide official passport and National Identity Card (NIC) photographs. Our services also include professional photography for Postal ID cards, Pension ID cards, and visa photographs tailored to meet the specific requirements of any country.</p>
                <p className='text-xl '>At Gobi Studio, we are committed to capturing moments with authenticity, precision, and a touch of elegance.</p>
            </div>

            <div className="mt-8 not-italic pl-15">
                <p className="text-2xl text-white font-bold font-sans mb-4 pl-20 py-2">Our Services</p>
                <div className=" space-y-4 grid grid-cols-2 gap-2 justify-center pl-24">
                    {services.map((service, index) => (
                        <Link
                            ...
                            duration={600}
                            offset={-70}
                            className="flex cursor-pointer items-center gap-3 hover:text-orange-600 transition-colors duration-300"
                        >
                        {service.icon}
                        <p className="text-lg text-textColor">{service.name}</p>
                        </Link>
                    ))}
                </div>
            </div>
        ...
        <div className='w-1/2 flex justify-center '>
            <img src={Owner} alt="Gobi Studio - Gobi" className='w-[70%] h-[700px] rounded-xl'></img>
        </div>
    </section>
}